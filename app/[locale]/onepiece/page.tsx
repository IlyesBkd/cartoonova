...
const Step = ({ n, title }: { n: number; title: string }) => (
  <div className="flex items-center gap-3 mb-6">
    <span className="flex-shrink-0 w-10 h-10 rounded-full bg-black text-amber-500 flex items-center justify-center font-black text-lg border-2 border-black">{n}</span>
    <h3 className="text-xl font-black text-black uppercase">{title}</h3>
  </div>
);

const Check = () => (
  <span className="absolute top-2 right-2 w-7 h-7 rounded-full bg-amber-500 text-black flex items-center justify-center text-xs font-black z-10 border-2 border-black">✓</span>
);
...
export default function OnePiecePage() {
  ...
  const orderDescription = `One Piece Wanted · ${format === "fullbody" ? t("fullbody") : t("portrait")} · ${people} pirate${people > 1 ? "s" : ""}${animals > 0 ? ` + ${animals} animal${animals > 1 ? "s" : ""}` : ""} · ${prints[selectedPrint]?.label || t("digital")}`;
  ...
  return (
    <>
      <div className="min-h-screen bg-white">
        ...
        {/* ═══ HERO avec VIDÉO ═══ */}
        <section className="relative pt-28 sm:pt-36 pb-14 sm:pb-20 bg-amber-500 border-b-4 border-black overflow-hidden">
              ...
              <div className="flex-1 w-full max-w-xl flex flex-col gap-4">
                <div className="bg-white p-3 border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                  <div className="aspect-[4/3] rounded-lg overflow-hidden border-2 border-black">
                    {heroPhoto === 0 ? (
                      <video
                        src="/onepiece/wanted_produit/1.mp4"
                        autoPlay
                        loop
                        muted
                        playsInline
                      ...
                      <img src={photos[heroPhoto - 1]} alt="Aperçu produit" className="w-full h-full object-cover" />
                    )}
                  </div>
                </div>
                <div className="grid grid-cols-6 gap-2">
                  ...
                  >
                    <video src="/onepiece/wanted_produit/1.mp4" muted className="w-full h-full object-cover" />
        ...
        <section className="py-14 sm:py-20 border-b-4 border-black">
          <W>
            <div className="text-center mb-10">
              <h2 className="text-3xl sm:text-4xl font-black text-black uppercase mb-2">{t("customizeTitle")} <span className="text-amber-500">{t("customizeHighlight")}</span></h2>
              <p className="text-black/50 font-bold max-w-lg mx-auto">{t("customizeSubtitle")}</p>
            </div>

            <div className="relative z-10 max-w-5xl mx-auto bg-amber-50 border-4 border-black rounded-3xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-6 sm:p-10 flex flex-col gap-14">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-5">
                  {([
                    { key: "portrait" as const, label: t("portrait"), desc: t("portraitDesc"), extra: t("included"), icon: "🏴‍☠️" },
                    { key: "fullbody" as const, label: t("fullbody"), desc: t("fullbodyDesc"), extra: prices ? `+${formatPrice(prices.fullbodyExtra)}` : "+20 €", icon: "🧍" },
                  ]).map((o) => (
                    <button key={o.key} onClick={() => { setFormat(o.key); trackOptionSelected("format", o.key, o.key === "fullbody" ? (prices?.fullbodyExtra || 0) : 0); }} className={`relative cursor-pointer text-left rounded-2xl p-6 transition-all duration-200 border-4 ${format === o.key ? "border-black bg-amber-500 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]" : "border-black/30 bg-white hover:border-black hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"}`}>
                      {format === o.key && <Check />}
                      ...
                      <p className="text-sm font-black text-amber-600 mt-3">{o.extra}</p>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-10">
                  <div className="flex-1">
                    <p className="text-sm font-black uppercase text-black mb-3">🏴‍☠️ {t("people")}</p>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="w-16 h-12 rounded-xl bg-amber-500 text-black font-black text-xl flex items-center justify-center border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">{people}</div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="w-16 h-12 rounded-xl bg-amber-500 text-black font-black text-xl flex items-center justify-center border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">{animals}</div>
              ...
              <div>
                ...
                <div className="border-4 border-dashed border-black rounded-2xl p-8 sm:p-12 text-center bg-white hover:bg-amber-50 transition-all duration-200">
                  <input
                    type="file"
                    id="photos-upload-onepiece"
                    multiple
                    ...
                    className="hidden"
                  />
                  <label htmlFor="photos-upload-onepiece" className="flex flex-col items-center gap-4 cursor-pointer">
                    <div className="w-20 h-20 rounded-2xl bg-amber-500 border-2 border-black flex items-center justify-center group-hover:scale-110 transition-transform shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                    ...
                    <button 
                      onClick={() => document.getElementById('photos-upload-onepiece')?.click()}
                      className="mt-2 bg-amber-500 text-black font-black text-sm uppercase px-8 py-3.5 rounded-full border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] active:translate-y-1 active:shadow-none transition-all cursor-pointer"
                    >
                      {t("uploadBtn")}
                    </button>
                  </label>
                  {uploading && (
                    <div className="mt-4 flex flex-col items-center gap-2">
                      <div className="w-12 h-12 border-4 border-black border-t-amber-500 rounded-full animate-spin" />
              ...
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <Step n={4} title={t("step5")} />
                  <span className="text-2xl animate-bounce">⬇️</span>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {prints.map((o, i) => (
                    <button key={i} onClick={() => { setSelectedPrint(i); trackOptionSelected("print", o.label, o.price); }} className={`relative cursor-pointer rounded-xl overflow-hidden transition-all duration-200 border-2 group ${selectedPrint === i ? "border-black bg-amber-100 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]" : "border-black/30 bg-white hover:border-black"}`}>
                      ...
                      <div className="p-2 text-center">
                        ...
                        <p className="text-amber-600 font-black text-sm">{o.price === 0 ? t("includedLabel") : formatPrice(o.price)}</p>
                      </div>
                    </button>
                  ))}
                </div>
        ...
        {/* ═══ GALERIE ═══ */}
        <section className="bg-red-600 py-14 sm:py-20 border-b-4 border-black">
          <W>
            <div className="text-center mb-10">
            ...
            <div className="flex flex-col gap-4">
              {faqData.map((f, i) => (
                <div key={i} className="border-4 border-black rounded-2xl overflow-hidden shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  <button onClick={() => setOpenFaq(openFaq === i ? -1 : i)} className="w-full flex items-center justify-between gap-4 px-5 py-4 bg-red-600 text-white text-left cursor-pointer hover:bg-red-700 transition-colors">
                  ...
                  {openFaq === i && <div className="px-5 py-4 bg-amber-50 text-black/70 text-sm font-bold leading-relaxed border-t-4 border-black">{f.a}</div>}
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ═══ AVIS ═══ */}
        <section className="bg-amber-500 py-14 sm:py-20 border-b-4 border-black">
          <W>
              ...
              {reviews.map((r, i) => (
                <div key={i} className="bg-white border-4 border-black rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] overflow-hidden flex flex-col hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-all duration-300">
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    <p className="text-amber-500 text-lg mb-2 font-black">★★★★★</p>
