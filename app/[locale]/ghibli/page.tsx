...
export default function ProductPage() {
  ...
  const { trackOptionSelected, trackPhotoUploaded, trackCheckoutStarted } = useProductTracking(PRODUCT_CONFIGS.ghibli);
  ...
  return (
    <>
      <ProductJsonLd product={{
        name: PRODUCT_STYLE_META.ghibli.name,
        description: PRODUCT_STYLE_META.ghibli.description,
        brand: "Cartoonova",
        image: PRODUCT_STYLE_META.ghibli.image,
        sku: PRODUCT_STYLE_META.ghibli.sku,
        offers: {
      ...
      <div className="min-h-screen bg-emerald-50 text-gray-900 overflow-x-hidden">
        ...
        <div className={`fixed bottom-24 lg:bottom-6 left-4 z-50 transition-all duration-500 ${toastVisible ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0 pointer-events-none"}`}>
          <div className="bg-white rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] px-4 py-3 flex items-center gap-3 max-w-xs">
            <div className="w-8 h-8 rounded-full bg-emerald-400 border-2 border-black flex items-center justify-center text-sm">🎨</div>
            <div>
        ...
        {/* ═══ HERO ═══ */}
        <section ref={heroRef} className="relative bg-emerald-50 overflow-hidden">
          {/* Decorative cartoon elements */}
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            <div className="absolute top-16 -left-6 w-20 h-20 rounded-full bg-emerald-300/20 blur-sm" />
            ...
            <div className="absolute top-40 right-[20%] w-8 h-8 rounded-full bg-emerald-400/20" />
          </div>
            ...
            <div className="grid lg:grid-cols-2 gap-6 lg:gap-10 items-start">
              ...
              <div className="relative order-1">
                <div className="relative aspect-[4/5] max-w-lg mx-auto rounded-2xl overflow-hidden border-3 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                  <Image src={GALLERY_PHOTOS[activePhoto]} alt="Portrait Studio Ghibli personnalisé" fill className="object-cover transition-all duration-700" priority sizes="(max-width: 768px) 92vw, 44vw" />
                </div>
                <div className="flex gap-2 justify-center mt-4">
                  {GALLERY_PHOTOS.slice(0, 6).map((photo, i) => (
                    <button key={i} onClick={() => setActivePhoto(i)} className={`w-14 h-14 rounded-lg overflow-hidden border-2 border-black transition-all duration-200 ${activePhoto === i ? "bg-emerald-400 scale-110 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]" : "opacity-60 hover:opacity-90 hover:scale-105"}`}>
              ...
              <div className="order-2 text-center lg:text-left lg:pt-6">
                <div className="inline-flex items-center gap-2 mb-5 bg-white border-2 border-black rounded-full px-3 py-1.5 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
                  <div className="flex">{[...Array(5)].map((_, i) => <span key={i} className="text-emerald-400 text-base">★</span>)}</div>
                ...
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black leading-[1.1] mb-4 uppercase">
                  {t("heroTitle1")} <span className="text-emerald-500 inline-block -rotate-1">{t("heroTitle2")}</span>
                </h1>
                ...
                <button onClick={scrollToConfig} className="group w-full sm:w-auto bg-emerald-400 text-black font-black text-lg px-12 py-4 rounded-xl border-3 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[3px] hover:translate-y-[3px] active:shadow-none active:translate-x-[5px] active:translate-y-[5px] transition-all">
            ...
            <div className="max-w-5xl mx-auto px-4 flex items-center justify-center gap-4 sm:gap-8 lg:gap-14 flex-wrap">
              {[{ v: "2,500+", l: tp("portraitsCreated") }, { v: "4.9★", l: tp("averageRating") }, { v: "48h", l: tp("delivery") }, { v: "100%", l: tp("satisfactionGuarantee") }].map((s, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-emerald-400 font-black text-lg sm:text-xl">{s.v}</span>
        ...
        <section className="py-10 bg-emerald-50">
          <div className="max-w-4xl mx-auto px-4 grid grid-cols-3 gap-5 text-center">
            {[
              ...
              <div key={i} className={`flex flex-col items-center bg-emerald-100 border-2 border-black rounded-2xl p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${s.rotate} hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all`}>
                <div className="w-14 h-14 rounded-full bg-emerald-400 border-2 border-black flex items-center justify-center text-2xl mb-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">{s.icon}</div>
        ...
        <section ref={configRef} className="relative py-10 sm:py-14 bg-emerald-50">
          {/* Decorative cartoon elements */}
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            <div className="absolute top-10 right-[5%] w-24 h-24 rounded-full bg-emerald-300/15" />
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-1 mb-8 max-w-lg mx-auto lg:mx-0">
              {STEPS.map((_, i) => (
                <div key={i} className="flex-1"><div className={`h-1.5 rounded-full transition-all duration-500 ${i <= activeStep ? "bg-emerald-400" : "bg-gray-200"}`} /></div>
              ))}
                  ...
                  return (
                    <div key={idx} className={`rounded-2xl border-2 border-black transition-all duration-300 ${isOpen ? "bg-white shadow-[5px_5px_0px_0px_rgba(250,204,21,0.6)]" : isDone ? "bg-green-50/80 shadow-[3px_3px_0px_0px_rgba(34,197,94,0.3)]" : "bg-white shadow-[3px_3px_0px_0px_rgba(0,0,0,0.1)] hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.15)]"}`}>
                      <button onClick={() => setActiveStep(idx)} className="w-full flex items-center gap-3 px-5 py-4 text-left">
                        <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-black transition-all ${isDone ? "bg-green-500 text-white shadow-sm" : isOpen ? "bg-emerald-400 text-black shadow-sm" : "bg-gray-100 text-gray-400"}`}>{isDone ? "✓" : step.num}</span>
                        ...
                        <svg className={`w-4 h-4 transition-transform duration-200 ${isOpen ? "rotate-180 text-emerald-500" : "text-gray-400"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                      </button>
                          ...
                          {idx === 0 && (<div>
                            <div className="grid grid-cols-2 gap-3">
                              {([{ key: "portrait" as const, label: tp("portraitBust"), extra: tp("included"), icon: "👤" }, { key: "fullbody" as const, label: tp("fullbodyLabel"), extra: prices ? `+${formatPrice(prices.fullbodyExtra)}` : "", icon: "🧍" }]).map((o) => (
                                <button key={o.key} onClick={() => { setFormat(o.key); trackOptionSelected("format", o.key, o.key === "fullbody" ? (prices?.fullbodyExtra || 0) : 0); }} className={`relative text-center rounded-xl p-4 transition-all border-2 border-black ${format === o.key ? "bg-emerald-100 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]" : "bg-white shadow-[3px_3px_0px_0px_rgba(0,0,0,0.15)] hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)]"}`}>
                                  <span className="text-2xl block mb-2">{o.icon}</span>
                                  <p className="font-bold text-gray-900 text-sm">{o.label}</p>
                                  <p className="text-xs text-emerald-500 font-semibold mt-1">{o.extra}</p>
                                </button>
                              ))}
                            </div>
                            <button onClick={() => goNextStep(0)} className="mt-4 w-full bg-emerald-400 text-black font-black text-sm py-3 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all uppercase">{tp("continue")}</button>
                          </div>)}
                          ...
                          {idx === 1 && (<div className="space-y-4">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="w-10 h-9 rounded-lg bg-emerald-400 text-black font-bold flex items-center justify-center">{people}</span>
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="w-10 h-9 rounded-lg bg-emerald-400 text-black font-bold flex items-center justify-center">{animals}</span>
                            ...
                            <button onClick={() => goNextStep(1)} className="w-full bg-emerald-400 text-black font-black text-sm py-3 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all uppercase">{tp("continue")}</button>
                          </div>)}
                          {/* STEP 2: Upload photo */}
                          {idx === 2 && (<div className="relative">
                            <Confetti active={showConfetti} />
                            {!sendPhotoLater && (<div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-emerald-400 hover:bg-emerald-100/30 transition-all">
                            ...
                            <button onClick={() => setSendPhotoLater(!sendPhotoLater)} className={`mt-3 w-full text-left text-sm p-3 rounded-lg border-2 transition-all ${sendPhotoLater ? "border-emerald-400 bg-emerald-100" : "border-gray-200 bg-gray-50 hover:border-gray-300"}`}>
                            ...
                            <button onClick={() => goNextStep(2)} className="mt-3 w-full bg-emerald-400 text-black font-black text-sm py-3 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all uppercase">{tp("continue")}</button>
                          </div>)}
                          {/* STEP 3: Print Options + Gift */}
                          {idx === 3 && (<div>
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                              {prints.map((o, i) => (
                                <button key={i} onClick={() => { setSelectedPrint(i); trackOptionSelected("print", o.label, o.price); }} className={`relative rounded-xl overflow-hidden border-2 transition-all ${selectedPrint === i ? "border-emerald-400 bg-emerald-50 shadow-sm" : "border-gray-200 bg-white hover:border-gray-300"}`}>
                                  <div className="aspect-square relative"><Image src={o.img} alt={o.label} fill className="object-cover" sizes="120px" /></div>
                                  <div className="p-2 text-center"><p className="font-bold text-gray-900 text-xs">{o.label}</p><p className="text-emerald-500 font-bold text-xs">{o.price === 0 ? tp("included") : `+${formatPrice(o.price)}`}</p></div>
                                </button>
                              ))}
                            </div>
                            <div className="mt-4">
                              <button onClick={() => setIsGift(!isGift)} className={`w-full flex items-center gap-3 p-3 rounded-lg border-2 transition-all ${isGift ? "border-emerald-400 bg-emerald-100" : "border-gray-200 bg-gray-50 hover:border-gray-300"}`}>
                                ...
                                <div className={`w-10 h-6 rounded-full transition-colors flex items-center ${isGift ? "bg-emerald-400 justify-end" : "bg-gray-200 justify-start"}`}><div className="w-5 h-5 rounded-full bg-white shadow mx-0.5" /></div>
                              </button>
                              {isGift && <textarea value={giftMessage} onChange={(e) => setGiftMessage(e.target.value)} placeholder={tp("giftMessagePlaceholder")} className="mt-2 w-full border border-gray-200 rounded-lg p-3 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:border-emerald-400 resize-none" rows={2} />}
                            </div>
                            ...
                            <div className="mt-4 relative bg-white rounded-2xl border-2 border-black p-3 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
                              ...
                              <div className="flex">{[...Array(5)].map((_, j) => <span key={j} className="text-emerald-400 text-xs">★</span>)}</div>
                ...
                <div className="sticky top-20">
                  <div className="bg-gray-900 rounded-2xl p-6 text-white shadow-2xl">
                    ...
                    <div className="space-y-2.5 text-sm mb-5">
                      ...
                      {isGift && <div className="flex justify-between"><span className="text-gray-400">{tp("giftLabel")}</span><span className="font-semibold text-emerald-400">{tp("giftYes")}</span></div>}
                    </div>
                    <div className="border-t border-white/10 pt-4 mb-5">
                      <div className="flex items-end justify-between">
                        ...
                        <span className="text-4xl font-black text-emerald-400"><AnimatedPrice value={total} formatter={formatPrice} /></span>
                      </div>
                    </div>
                    <button onClick={() => { setShowCheckout(true); trackCheckoutStarted(total, currency, { format, people, animals, background: backgrounds[selectedBg]?.label || "Standard", printOption: prints[selectedPrint]?.label || "Digital" }); }} className="w-full bg-emerald-400 text-black font-black text-lg py-5 rounded-xl border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[3px] hover:translate-y-[3px] active:shadow-none active:translate-x-[5px] active:translate-y-[5px] transition-all uppercase">
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-white border-t border-gray-200 px-4 py-3 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
          <div className="flex items-center justify-between gap-3">
            ...
            <button onClick={() => { setShowCheckout(true); trackCheckoutStarted(total, currency, { format, people, animals, background: backgrounds[selectedBg]?.label || "Standard", printOption: prints[selectedPrint]?.label || "Digital" }); }} className="flex-1 max-w-xs bg-emerald-400 hover:bg-emerald-500 text-black font-black text-sm py-3.5 rounded-xl transition-all animate-subtle-pulse">
        ...
        {/* ═══ GALLERY ═══ */}
        <section className="py-12 sm:py-16 bg-emerald-50 relative overflow-hidden">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute -top-4 right-[10%] w-20 h-20 rounded-full bg-emerald-300/10" />
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-10">
              ...
              <div className="flex items-center justify-center gap-2 mt-3">
                <div className="flex gap-0.5">{[...Array(5)].map((_, i) => <span key={i} className="text-emerald-400 text-lg">★</span>)}</div>
            ...
            <div className="bg-white border-2 border-black rounded-2xl p-6 mb-6 shadow-[5px_5px_0px_0px_rgba(0,0,0,1)]">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center font-black text-white text-lg">{reviews[0]?.name.charAt(0)}</div>
                <div>
                  <p className="font-black text-gray-900">{reviews[0]?.name}</p>
                  <div className="flex items-center gap-1">{[...Array(5)].map((_, j) => <span key={j} className="text-emerald-400 text-sm">★</span>)}<span className="text-green-600 text-xs font-semibold ml-1.5 bg-green-100 px-1.5 py-0.5 rounded">{tp("verified")}</span></div>
                </div>
              </div>
              ...
              {reviews.slice(1).map((review, i) => (
                <div key={i} className="bg-white rounded-xl p-5 border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all">
                  <div className="flex items-center gap-2.5 mb-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-400 to-teal-400 flex items-center justify-center font-bold text-white text-sm">{review.name.charAt(0)}</div>
                    <div><p className="font-bold text-gray-900 text-sm">{review.name}</p><div className="flex items-center gap-0.5">{[...Array(5)].map((_, j) => <span key={j} className="text-emerald-400 text-sm">★</span>)}<span className="text-gray-400 text-xs ml-1.5">{tp("verified")}</span></div></div>
                  </div>
              ...
              {faqData.map((faq, i) => (
                <div key={i} className={`bg-white rounded-xl border-2 border-black overflow-hidden transition-all duration-200 ${openFaq === i ? "shadow-[4px_4px_0px_0px_rgba(250,204,21,0.8)]" : "shadow-[3px_3px_0px_0px_rgba(0,0,0,0.1)] hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.15)]"}`}>
                  <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between gap-3 px-5 py-4 text-left">
                    ...
                    <span className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-200 ${openFaq === i ? "bg-emerald-400 text-black rotate-180" : "bg-gray-100 text-gray-400"}`}>
        ...
        {/* Wavy separator */}
        <svg viewBox="0 0 1440 40" className="w-full -mb-1 text-emerald-400" preserveAspectRatio="none"><path fill="currentColor" d="M0,20 C360,0 720,40 1080,20 C1260,10 1380,30 1440,20 L1440,40 L0,40 Z" /></svg>

        {/* ═══ FINAL CTA ═══ */}
        <section className="relative py-14 sm:py-20 bg-emerald-400 overflow-hidden border-t-3 border-black">
          <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="max-w-3xl mx-auto px-4 text-center relative">
            ...
            <h2 className="text-3xl sm:text-4xl font-black text-black mb-3 uppercase"><span className="inline-block -rotate-1">{tp("readyToEnterGhibli")}</span></h2>
            <p className="text-base text-black/60 mb-8 max-w-lg mx-auto">{tp("joinClients")}</p>
            <button onClick={scrollToConfig} className="inline-flex items-center gap-2 bg-black text-emerald-400 font-black text-lg px-10 py-5 rounded-xl border-3 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,0.3)] hover:shadow-[3px_3px_0px_0px_rgba(0,0,0,0.3)] hover:translate-x-[3px] hover:translate-y-[3px] active:shadow-none active:translate-x-[6px] active:translate-y-[6px] transition-all uppercase">
              {tp("createMyPortrait")}
